...
- Computadora portatil **BMAX B1** con Ubuntu Linux
...
### 2.4 Sensores infrarrojos (sensor-a y sensor-b)

- Dispositivos ESP32 con sensor infrarrojo ABT-30
- **sensor-a** mide en la pista A
- **sensor-b** mide en la pista B
- Se comunican por **radio LoRa** (433 MHz) con el gateway, no por WiFi
- Detectan el cruce del auto y mandan el evento al sistema
- Funcionan con bateria o alimentacion externa

### 2.5 Semaforo

- Luces de semaforo (rojo / verde) para la largada
- Se controla por radio LoRa desde el gateway
- Cuando el operador pulsa "Armar largada" en la app: enciende rojo 3 segundos, luego verde 5 segundos
...
> Los sensores miden por rayo infrarrojo. Deben estar alineados (emisor y receptor uno frente al otro), sin obstaculos entre ellos.
...
3. La pantalla muestra la lista de todos los dispositivos con su estado:
   ...
   - **Punto gris = Offline** (no se ha comunicado en los ultimos 30 segundos)
...
9. Cuando los autos esten en la linea de largada y listos, tocar **"ARMAR LARGADA"** (boton verde)
   - El semaforo enciende en **rojo durante 3 segundos**
   - Luego cambia a **verde durante 5 segundos** (los autos deben arrancar al ver el verde)
   - Los sensores quedan activos por 20 segundos para detectar el cruce de largada
...
11. Cuando los autos van llegando, tocar **"ARMAR LLEGADA"** (boton amarillo)
    - Los sensores quedan activos por 20 segundos para detectar la llegada
...
### 5.3 Que hacer si un auto abandona (DNF)

Si un auto no puede terminar la pista por problemas mecanicos u otro motivo:

1. Seleccionar la tripulacion en la lista
2. Tocar **"Anular vuelta"** (boton rojo en la parte inferior)
3. Confirmar
4. La vuelta queda marcada como **NULA**
5. Si el auto no puede correr mas en toda la fecha, la tripulacion queda como **DNF** y recibe 1 punto en el campeonato

> Una vuelta nula no cuenta para el ranking. Si la tripulacion tiene otras vueltas validas, se usa la mejor.

### 5.4 Como anular una vuelta ya guardada
...
| Punto gris + texto "Offline" | Dispositivo no se comunica hace mas de 30 segundos |
...
- **Voltaje:** tension de alimentacion del dispositivo. Debe estar cerca de 5.0V
...
1. **El sensor estaba deshabilitado:** los sensores solo estan activos por 20 segundos despues de tocar "Armar largada" o "Armar llegada". Si el auto tardo mas de 20 segundos en cruzar, el sensor ya se desactivo.
   - Solucion: volver a tocar "Armar llegada" y esperar que el auto llegue

2. **El sensor o gateway esta offline:** verificar en el panel de dispositivos
   - Si el gateway esta offline: el sensor tampoco funciona. Reiniciar el gateway
   - Si el sensor esta offline: reiniciar el sensor

3. **Desalineacion del sensor:** el rayo infrarrojo no esta apuntando bien
   - Verificar que el emisor y el receptor esten bien alineados
   - Los autos pasan por enmedio del rayo

4. **Respaldo siempre disponible:** ingresar el tiempo manualmente. Anotar el tiempo en un papel en el momento del cruce y cargarlo despues.
...
4. Placa: `ESP32S3 Dev Module`, USB CDC On Boot: `Enabled`
...
- **Polling dispositivos:** cada 2 segundos
- **Heartbeat dispositivos:** cada 5 segundos
